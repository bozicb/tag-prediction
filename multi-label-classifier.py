...
def multi_label_classification(data,books=True,test_size=.2):
    ...
    if books:
        ...
        count_vectorizer_books = CountVectorizer(stop_words="english", tokenizer=tokenize, ngram_range=(1, 3), max_features=10000, analyzer="word")
        ...
        tfidf_vectorizer_books = TfidfVectorizer(stop_words="english", tokenizer=tokenize, ngram_range=(1, 3),
        ...
        cv_train_x = cv_train_x_content 
        cv_test_x = cv_test_x_content
        tv_train_x = tv_train_x_content
        tv_test_x = tv_test_x_content
    ...
    # Making predictions
    cv_pred = count_vectorizer_model.predict(cv_test_x.toarray())
    tv_pred = tfidf_vectorizer_model.predict(tv_test_x.toarray())
    ...
    predictions=pd.concat([test['tags'].reset_index(),predictions['predicted']],axis=1,keys=['original','predicted'],ignore_index=True)
